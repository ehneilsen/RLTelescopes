...
class SqueScheduler(Scheduler):
    ...
    def update(self, start_date, end_date=None):
        ...
        time_per_action = (
            self.config.getfloat("actions", "exposure_time")
            + self.obsprog.slew_rate * n_actions / 360
        )
        n_steps = int((length * 60 * 60) / time_per_action)
        ...
        for night in range(nights):
            ...
            for action in range(n_actions):
                ...
                iteration_actions = [
                    select_action for _ in range(int(n_steps_per_action))
                ]
    ...
    def calculate_action(self, **kwargs):
        ...
        if len(non_zero) == 0:
            ...
            action = non_zero[non_zero["reward"] == non_zero["reward"].min()].to_dict(
                "records"
            )[0]
...
if __name__ == "__main__":
    scheduler_config_path = os.path.abspath("train_configs" "/default_schedule.conf")

...
class ObservationProgramGenerator(Sequence):
    def __init__(self,
        ...
        self.actions = pd.concat([
            self.generate_action_set(bands, n_observation_sites, ra_range, delc_range)
            for _
            in range(n_observation_chains)
        ])
    ...
    def generate_action_set(self, bands, n_observation_sites, ra_range, delc_range):
        ...
        sites = [
            [ra_arrangement[ra_index],
             delc_arrangement[ra_index]]
        ]
        ...
        for band in bands:
            actions = pd.concat(
                [actions,
                 pd.DataFrame({"ra": sites[:,0].astype(np.float32), "delc": sites[:,1].astype(np.float32), "band": band})]
            )
        return actions
    ...
    def __getitem__(self, item):
        ...
        return self.data_generation(batch_actions.to_dict(orient="records")), batch_actions[["ra", "delc"]].values
    ...
    def data_generation(self, actions):
        ...
        for action in actions:
            ...
            current_observation = current_observation if \
                self.included_variables is None else current_observation[self.included_variables]
            current_observation['mjd'] = self.observation_program.mjd

            self.observation_program.update_observation(ra, delc, band, exposure_time=300)

            new_observation = pd.DataFrame(self.observation_program.state, index=[0])
            new_observation = new_observation if \
                self.included_variables is None else new_observation[self.included_variables]
        ...
        observation_chain_t0 = observation_chain[[col for col in observation_chain.columns if "t0" in col]].values
        observation_chain_t1 = observation_chain[[col for col in observation_chain.columns if "t1" in col]].values
